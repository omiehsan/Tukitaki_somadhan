      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) => GestureDetector(
          onTap: () {
            ...
            print(catName);
            Get.to(AllJobsByFilterScreen(
              filterByCat: catName,
              pageTitle: "$catName - All Jobs",
            ));
          },
          child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              const Icon(Icons.circle, color: Colors.green, size: 8.0),
                              const SizedBox(width: 3.0),
                              Text("${catItems[index].catExperts}", style: const TextStyle(fontWeight: FontWeight.bold)),
                              const SizedBox(width: 3.0),
                              const Text(TTS_Experts),
                            ],
                          ),
                          ...
                          Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              const Icon(Icons.check, color: TTS_LogoColor, size: 15.0),
                              const SizedBox(width: 3.0),
                              Text("${catItems[index].catJobs}", style: const TextStyle(fontWeight: FontWeight.bold)),
                              const SizedBox(width: 3.0),
                              const Text(TTS_Jobs_Posted),
                            ],
                          ),
                        ],
                      ),
